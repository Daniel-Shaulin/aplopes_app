...
class AuthController extends GetxController {
  ...
  Future<void> singUp() async {
    ...
    GenericResult result = await authRepository.signUpUser(user);
    isLoadingSignUp.value = false;
    result.when(sucess: (user) {
      utilsService.showToast(message: 'Usuário cadastrado com sucesso!', isError: false);
    }, error: (error) {
  ...
  Future<void> validateToken() async {
    ...
    if (token == null) {
      ...
      String? expirationToken =
          await utilsService.getLocalData(key: StorageKeys.expirationToken);
      String? refreshToken =
          await utilsService.getLocalData(key: StorageKeys.refreshToken);
      if (expirationToken != null &&
          DateTime.parse(expirationToken).compareTo(DateTime.now()) > 0) {
  ...
  void saveTokenAndProssedToBase() {
    ...
    utilsService.saveLocalData(
        key: StorageKeys.expirationToken, data: date.toString());
    utilsService.saveLocalData(
        key: StorageKeys.refreshToken, data: token.refreshToken!);
    utilsService.saveLocalData(
        key: StorageKeys.userId, data: token.userId!.toString());
    Get.offAllNamed(PagesRoutes.baseRout);
  }
